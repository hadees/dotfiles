...
@test "doctor: runs every doctor in order" {
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == "shell:   zsh "*"== git-doctor"*"== gh-doctor"*"== claude-doctor"*"== wrangler-doctor"*"== hermes-doctor"*"== tailnet-doctor"*"== workspace-doctor"*"== iterm2-doctor"* ]]
  [[ "$output" == *"author:  Octo Person <octo@example.com>"* ]]
}
...
@test "doctors: every reachable helper is covered by the defined: line" {
  ...
  run zsh -c '
    ...
    typeset -A wrapped=(claude claude workspace "" wrangler wrangler gh gh hermes hermes tailnet "ssh scp sftp curl tailnet-as" git "" iterm2 "")
    local d h n=0 bad=0
    for d in claude workspace wrangler gh hermes tailnet git iterm2; do
