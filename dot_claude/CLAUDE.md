...
- Two gh accounts are logged in: work `work-account` (work-org/work-org-two orgs)
  and personal `hadees`. hadees is NOT an work-org member — it sees only
...
- A `gh()` wrapper in ~/.functions pins the token per invocation: an explicit
  work-org/work-org-two/hadees owner in the arguments wins, else the repo's
  origin host alias (`github-work-account:` / `github-hadees:`), else the active
  account. It never runs `gh auth switch`.
- If org-scoped gh output looks thin or wrong, don't trust the active account:
  pin it explicitly with `GH_TOKEN=$(gh auth token --user work-account) gh …`.
